...
Full component detail is in the E5.1 section of `xantham-system-v33.md`. These are layered on top of the gate, not a replacement for it.
...
- **Verify the blueprint cryptographically.** Every commit to `main` regenerates `CHECKSUMS.sha256` with SHA256 hashes of the seven published artifacts (`xantham-system-v33.md`, `xantham-templates-v33.md`, `LICENSE`, `README.md`, `SECURITY.md`, `ARCHITECTURE.md`, `COMPARISON.md`). Run `bash scripts/verify-blueprint.sh` (or the curl one-liner in the README) to confirm the bytes you fetched match the bytes the maintainer published.
- **Pin to a commit SHA you have reviewed.** The README's install section supports pinned-SHA URLs (`/<sha>/xantham-system-v33.md` instead of `/main/xantham-system-v33.md`). Pinning to a SHA defeats the "I trusted main, then the repo was compromised after my audit" attack and makes the install reproducible.
...
Security fixes ship as new commits on `main` with a `security:` prefix in the commit message. Watch the repo (GitHub bell -> Custom -> Releases + Issues) to get notified. Major safety-gate changes are also called out in the changelog section of `xantham-system-v33.md` (or the equivalent file on the latest version).
