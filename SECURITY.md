...
- The version of the blueprint you are looking at (v36, v35, etc.)
...
Full component detail is in the E5.1 section of `xantham-system-v36.md`. These are layered on top of the gate, not a replacement for it.
...
- **Verify the blueprint cryptographically.** Every commit to `main` regenerates `CHECKSUMS.sha256` with SHA256 hashes of the published artifacts: the current landing file `xantham-system-v36.md`, the prior `xantham-system-v35.md`, `xantham-system-v34.md` and `xantham-system-v32.md` retained for reference, plus `xantham-templates-v32.md`, `LICENSE`, `README.md`, `SECURITY.md`, `ARCHITECTURE.md` and `COMPARISON.md`. Run `bash scripts/verify-blueprint.sh` (or the curl one-liner in the README) to confirm the bytes you fetched match the bytes the maintainer published.
- **Pin to a commit SHA you have reviewed.** The README's install section supports pinned-SHA URLs (`/<sha>/xantham-system-v36.md` instead of `/main/xantham-system-v36.md`). Pinning to a SHA defeats the "I trusted main, then the repo was compromised after my audit" attack and makes the install reproducible.
...
Security fixes ship as new commits on `main` with a `security:` prefix in the commit message. Watch the repo (GitHub bell -> Custom -> Releases + Issues) to get notified. Major safety-gate changes are also called out in the changelog section of `xantham-system-v36.md` (or the equivalent file on the latest version).
