...
- **`xantham-system-v36.md`** (~7600 lines). The landing file (current version). Install wizard, day-1 user experience docs, architecture reference, advanced patterns, troubleshooting catalogue. The human-readable half. The prior `xantham-system-v35.md`, `xantham-system-v34.md` and `xantham-system-v32.md` remain in the repo for reference (v33 was an internal-only bump; v36 is the current record). All four are covered by the checksum manifest.
- **`xantham-templates-v32.md`** (~15900 lines). The templates appendix. Every script body, hook template, settings.json variant, agent config, skill body, memory seed that the wizard generates. The wizard's install steps reference these by name; the user's Claude reads both files in sequence.
...
- **`.claude/skills/xantham-sync-habits/SKILL.md`, `xantham-memory/`, `xantham-spec-kit-bridge/`, `xantham-ai-seo/`, `xantham-21st-bridge/`**. Five skill bodies shipped as actual files in this repo, all fetchable by URL — `install-xantham-habits.sh` pulls all five. Three more (`xantham-orchestration`, `xantham-reflection`, `xantham-safety`) are generated fresh per-install by the full setup wizard rather than fetched by URL (they depend on your chosen orchestrator name and mode); see the Manifest section of `orchestration-habits.md` for which is which.
...
- **The second engine becomes a building hand, not just an advisor.** Earlier versions used a second model for review and second opinions. It now builds: it runs the same crew personas, writes on a feature branch inside a sandbox, and its own shell commands are gated by the same safety hook that gates everything else. One gate, two engines, no second policy to keep in sync. It authenticates on a normal chat subscription, so there is no metered API key in the loop.
- **Per-tier reasoning effort and a build-mode toggle.** Reasoning effort is set per tier rather than globally, so a cheap read and a hard architectural call stop costing the same. A single toggle moves the rig between review-only and build-capable, which makes write access a decision you make per session rather than a standing grant.
- **Tri-method research.** Research questions fan out across independent retrieval paths in parallel instead of trusting one, so a single thin or stale source can never be the whole answer.
- **One rulebook both engines read.** A conventions bridge keeps the repo's rules in a single file both engines load, instead of letting them drift into two dialects. A knowledge index sits beside it so a fresh session can find what the system already knows rather than rediscovering it.
- **Crew bodies tailored to their model.** A clone still shares its original's identity, role and memory directory, but its body is now written for the engine it runs on rather than copied across three tiers. The fast-model clone is written to do its best full work, not to be cheap. Every persona also gains a worked exemplar of its core deliverable, a machine-readable output contract, a runnable self-verification block, and a directive list of the tools that role should reach for. An advisor pairing hangs off the decision points, where a second read earns its cost.
- **Completeness pass (v36.6).** A full feature census reconciled the guide against what the reference install actually runs and closed the gaps between them. The deep-research pipeline is back in the guide, a user-level skills catalogue was added, the arsenal became a hard rule rather than a suggestion, and the reflexion layer gained a fifth category.
- **The master-build gauntlet generalises from two engines to many (v36.7).** One command runs the highest-assurance path your rig can reach. Every engine plans the same brief blind. None of them sees another plan until the reconcile step, because engines that can read each other converge, and a convergent second opinion is an echo. Each planner is routed to a different axis (spine, deep-reasoning architecture, backend, threat model), since the win is the axis an engine sits on rather than the headcount: route four planners at one axis and you have paid four times for one opinion. They then adversarially cross-verify before anything ships, and the reconcile step's real deliverable is the disagreements and risk-hotspots list. A genuine split between engines is a decision to make, not something to average away. The whole pipeline auto-scales, so a one-file fix skips the heavy stages.
- **Reactive model-defense layer.** Beside the three-bucket gate sits a set of hooks and deterministic scripts that close the data-loss and fabricated-completion failure modes that surfaced with newer model generations: the gate also blocks `git stash pop`/`apply`, chained stash-and-switch, `checkout`/`restore` over a dirty tree, and broad `git add -A`/`.`; a warn-only reply-verify gate flags "shipped / passing" claims with no verification recorded; a non-killing loop detector pings you when the same command repeats; transcript grounding substring-checks every quote against its source; and a pre-merge guard reports a branch's real deletions before you merge it. Every defense is a hook or a script, not a prompt rule. See the E5.1 section of `xantham-system-v36.md` and `SECURITY.md`.
