...
- **`xantham-system-v33.md`** (~5400 lines). The landing file. Install wizard, day-1 user experience docs, architecture reference, advanced patterns, troubleshooting catalogue. The human-readable half.
- **`xantham-templates-v33.md`** (~11400 lines). The templates appendix. Every script body, hook template, settings.json variant, agent config, skill body, memory seed that the wizard generates. The wizard's install steps reference these by name; the user's Claude reads both files in sequence.
...
v32 built a measurement + closed-loop slate and a self-improvement engine; v33 turns that engine from **present** into **proven** — the theme is *wire up + prove what's already built*, not build more. New standing bar: a capability isn't "done" until its effect is a logged number, not an assumption. Every mechanism below is toggleable and ships **OFF by default**.

- **Session continuity is now a first-class pillar.** A real compaction checkpoint (distill session state before a context compaction, re-inject + verify after), a completeness gate that blocks "go fresh" until every parked item / overdue task / new memory / uncommitted note is captured, auto-persist at the session boundary (commit + reconcile + write a pickup file the next greeting actually reads), and a **byte-capped** memory index (a line-cap doesn't stop a big index from silently truncating on load).
- **Cost tripwire wired onto every agent dispatch.** Register/complete on each dispatch fires cost-attribution + in-flight dedup + a fan-out width-cap + a run-tree at once, plus a per-run and per-day ceiling — the single highest-leverage move for anyone running heavy parallel dispatch on a metered or rate-limited plan.
- **Verification hardening.** Grounding now needs a real run signal instead of matching the command string (a read-only command used to count as "verified"); a reporter-confirm gate for operator-reported bugs.
- **Memory ranker on by default + an eval that can move.** A composite retrieval ranker (temporal-decay + entity-expansion + centrality) becomes the live default with a measured MRR lift, and the eval gains rank-sensitive MRR + nDCG@k so a ranking change is visible — a recall@k-only harness saturates at 100% and hides the improvement.
- **The self-improvement loop closes end-to-end.** An on-demand self-improvement orchestrator, a walkthrough reader/injector so a task's forensic write-back is actually loaded by the next related dispatch, a cadence run-due check at session start, and — behind an optional capped API key — a GEPA quality judge and an injection-scanned skill-authoring fork. Every key-gated piece defers at zero spend with no key.
- **Model-tier refinement.** Strongest model stays the default workhorse; add an on-demand fast/cheap clone tier for breadth and parallelism, and reserve a heaviest-model clone tier for the hard 20% on explicit say-so. Detailed in the blueprint's "Model and effort per agent" section.
- **Honesty guardrail (the most important one).** A model grading its own system is sycophantic — it designs rubrics that flatter itself. Any self-audit, best-in-class claim, or self-designed rubric must get an independent check (a different model, an external benchmark, or an adversarial red-team told to find where you're *worse*) before it's trusted or shown externally. Do not claim best-in-class externally until externally benchmarked.
- **Security first.** The secret-redaction helper's coverage was widened to modern high-risk key shapes (Google `AIza`/`ya29`, JWTs including managed-DB service-role keys, Twilio `AC`, SendGrid `SG.`) before anything else shipped, plus a worktree-cwd guard that stops a sub-agent from misrouting the main session's commits.
