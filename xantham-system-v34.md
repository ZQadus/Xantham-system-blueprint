...
> **New in v34 (2026-07-08) — the "nothing the operator says or is promised silently slips" slate.** v33 wired up and proved the engine; v34 closes a different failure class: things that only happen if the model *remembers* to act on them. A deterministic **open-commitments ledger** (a promise the orchestrator makes is recorded on send and reconciled before any "did you do X?" reply), a **standing-rules engine** (recurring directives like "every morning, do X" fire off a machine spine, never off the model recalling them), and a **discussed-ideas capture** seam (an idea raised in chat is filed, not lost). Plus the session-continuity persist chain relit end-to-end, a decision-point **advisor** pairing, and a GEPA quality-judge upgrade. Full detail in the Changelog. **The install flow is unchanged from v33** — same wizard, same mode picker, same templates appendix; v34 only expands the inventory.
...
### v34 - memory-integrity bulletproofing + the durable-directive engine (2026-07-08)

v33 wired the engine up and proved it. v34's theme is a different failure class: **anything the operator says, or is promised, that silently slips because the model has to remember to act on it.** Two real incidents drove it — a promised deliverable (a dashboard) got quietly deprioritised with no flag, and a standing directive ("when I say X, do Y") only fired when the model happened to recall it. The fix in both cases is the same shape: **detect deterministically, persist to a machine-readable spine, and fire off that spine — never off the model's curated memory.** Everything below is zero-LLM on its live path and inherits the same gating posture as the rest of the engine (propose-on-anything-uncertain, git-reversible, protected paths untouchable). Where a piece needs a hook installed, it ships **staged for a one-time out-of-band install** — an agent run cannot edit its own harness hooks, so the install step is a small command the operator runs in a real terminal.

- **Commitment integrity (memory-hardening Phase 1) — a promise can't silently slip.** A five-layer root-cause audit (run adversarially by a second model) traced the dropped-deliverable class to self-correcting loops that were all DARK in production. The deterministic fix has three parts: a `PostToolUse` **commitment-extractor** that reads every outbound reply and records each specific deliverable the orchestrator commits to into a durable **open-commitments ledger** (`memory/project_open_commitments_tracker.md` plus a machine spine); a **reconcile-question gate** that forces a reconcile against that ledger + the Telegram tail before the orchestrator answers any "did you / where's X / what's pending" question (so the answer is grounded, not asserted); and a **turn-contract commitment guarantee** at the Stop hook (a reply-reminder plus a contract-verify that a committed action actually left the turn). 44+ hook-test assertions. Staged under `scripts/memfix-phase1/` for the gated out-of-band install.

- **The standing-rules engine — durable directives fire without the model remembering.** The sibling fix to the commitment tracker for the same model-dependent-memory failure class. A deterministic **detect → persist → fire** pipeline for recurring directives ("when I say morning, go through my emails"; "every morning send the overnight summary"; "from now on always use the strong model"). Detection needs a **directive-verb plus a not-third-person heuristic** — not a bare keyword and not the words "hard rule" — so a genuine standing order is caught but an offhand mention is not. Persistence is a machine JSONL spine at `data/runtime/standing-rules.jsonl`, and firing reads **the spine, not model-curated markdown**, so a rule keeps working across sessions with zero dependence on the model recalling it. A `UserPromptSubmit` hook injects the matching rule as `additionalContext` when its keyword trigger fires (multiple `UserPromptSubmit` hooks stack — they all inject); schedule rules surface in the morning digest instead; "always" rules apply generally. An off-switch is mandatory — retiring a rule flips the status the fire path reads, so a rule can always be turned off. The human-readable canonical view lives in `memory/standing_rules.md`; the spine is authoritative. 75 assertions green.

- **Discussed-ideas capture — an idea raised in chat doesn't evaporate.** A zero-latency capture seam: a `PostToolUse`-async hook on the reply tool that reads the operator's INBOUND message from `data/runtime/inbound.txt` and files ideas or decisions raised in conversation so they survive the session. It nags on the operator's inbound only (never on the orchestrator's own verbose outbound), and its precision comes from explicit guards rather than from weakening the anchors — a firm promise already caught by the commitment ledger uses a **disjoint anchor set** so it is never double-captured. Staged install; 75 tests.

- **Session-continuity persist chain relit end-to-end.** v33 designed the continuity pillar; v34 confirms the SessionEnd auto-persist chain (which had silently gone dead ~June) is wired and firing again — commit + reconcile + write-a-pickup-file at the session boundary, with the explicit "new session" command syncing FIRST so nothing parked is lost across a fresh start. This is the durable spine the commitment ledger and the standing-rules engine both hang off; relighting it was the prerequisite for both.

- **GEPA quality judge: fractional score → binary verdict.** The deferred GEPA optimizer's quality judge was moved from a float score to a **binary pass/fail** verdict, so candidate prompts/configs rank crisply on the Pareto frontier instead of on noisy fractional scores that made near-ties indistinguishable. Dormant and forward-looking (still key-gated, still defers at zero spend with no key), tested on the branch. A small correctness upgrade to a component that only fully activates the day a dedicated API key is provisioned.

- **The advisor pairing — a second opinion at hard decision points.** Claude Code's **advisor** tool is wired as a per-decision escalation: at a genuinely hard decision point (before committing to an approach, when stuck, before declaring done) the main model can consult a stronger or peer advisor model that is forwarded the full transcript, so it sees exactly what the main model did. It is a **scalpel, not a default** — the orchestrator calls it only when a second opinion genuinely earns its cost, never on routine work, greetings, or one-word confirmations. The default `advisorModel` (in `.claude/settings.json`) is the strong workhorse model; a heavier model is escalated only as a per-task, single-session override on genuinely-hard multi-file work where the strong model plateaus, and **never** on security-sensitive work or credential/client transcripts (the heavier model's data-retention posture rules those out).

- **Fast-clone tier is now the near-flagship model.** The on-demand fast/cheap clone tier from v33 is now backed by the current fast-capable model (`<name>-<model>` naming, byte-identical persona, shares the original's agent-memory directory). It is the parallel/breadth workhorse for the bulk of build and edit work; the flagship model stays the default and the orchestrator, and the heaviest-model clone stays reserved for the hard 20% (security/auth, gnarly architecture, deep root-cause, schema/migrations) on explicit operator say-so only. An edit to a plain agent body is still mirrored into every twin (fast-clone and heavy-clone alike).

- **Productization pointer (deliberately NOT part of the install).** This orchestrator is also the base for a de-personalized, client-installable product build — the full crew, all skills, and the engine shipped as a clean starting point that someone else personalizes, with its own security / isolation / licensing surface. That productization is a separate track and is intentionally NOT part of this self-install blueprint, whose spine stays the personal orchestrator. Mentioned only so the architecture is honestly complete.
...
- **Outbound reply lint consolidated to a single neutral hook.** `.claude/hooks/cortana-output-lint.sh` is now the one source of truth for outbound reply style (canonical rules in `docs/output-discipline-rules.md`); there is no per-mode lint variant.
...
### Commitment integrity + durable directives (memory-hardening Phase 1, Advanced mode)

Three deterministic mechanisms close the "the model has to remember to act on it" failure class. All persist to a machine-readable spine and fire off that spine, never off the model's curated markdown, so a promise or a standing order keeps working across sessions with zero dependence on model recall. Each ships **staged for a one-time out-of-band install** (an agent run cannot edit its own harness hooks) with a hermetic self-test.

- **Open-commitments ledger.** A `PostToolUse` commitment-extractor reads every outbound reply and records each specific deliverable the orchestrator commits to into `memory/project_open_commitments_tracker.md` (plus a machine spine). A reconcile-question gate forces a reconcile against that ledger + the Telegram tail before the orchestrator answers any "did you / where's X / what's pending" question, and a turn-contract guarantee at the Stop hook verifies a committed action actually left the turn. Born from a real incident where a promised deliverable was quietly deprioritised with no flag. 44+ hook-test assertions.
- **Standing-rules engine.** A detect → persist → fire pipeline for recurring directives ("when I say morning, go through my emails"). Detection needs a directive-verb plus a not-third-person heuristic (not a bare keyword, and not the words "hard rule") so a genuine standing order is caught but an offhand mention is not; persistence is a JSONL spine at `data/runtime/standing-rules.jsonl`; firing is a `UserPromptSubmit` hook that injects the matching rule as `additionalContext` on its keyword trigger (schedule rules surface in the morning digest instead). Every rule has an off-switch — retiring it flips the status the fire path reads. The human-readable canonical view is `memory/standing_rules.md`; the spine is authoritative. 75 assertions.
- **Discussed-ideas capture.** A `PostToolUse`-async seam on the reply tool that files ideas or decisions raised in the operator's inbound message (read from `data/runtime/inbound.txt`) so they survive the session, with guards (a disjoint anchor set) so a firm promise already caught by the commitment ledger is never double-captured. It nags on the operator's inbound only, never on the orchestrator's own outbound. 75 tests.

The three share one design principle with the session-continuity spine (Section 12): the durable record is a file or JSONL the hooks read deterministically, so the behaviour survives a compaction, a fresh session, or the model simply forgetting.
...
The reply-output discipline this build relies on is enforced by a single neutral hook, `.claude/hooks/cortana-output-lint.sh` (a PreToolUse hook on the Telegram reply tool, canonical rules in `docs/output-discipline-rules.md`). It is the one source of truth for outbound style; no per-mode alias variant exists.
...
### The advisor pairing (decision-point second opinion, v34)

Claude Code's advisor tool pairs the main model with a stronger or peer advisor model at explicit decision points — before committing to an approach, when stuck, and before declaring done — forwarding the full conversation transcript so the advisor sees exactly what the main model did and every result it saw. It is a scalpel, not a default: the orchestrator invokes it only when a second opinion genuinely earns its cost (an irreversible decision, a security/payments surface, a large migration, or low confidence in its own answer), never on routine work, greetings, or one-word confirmations. The default `advisorModel` is set in `.claude/settings.json` to the strong workhorse model; a heavier model is escalated only as a per-task, single-session override on genuinely-hard multi-file work where the strong model plateaus, and never on security-sensitive work or on credential/client transcripts where the heavier model's data-retention posture rules it out. The advisor's guidance is weighted seriously but not blindly — primary-source evidence that contradicts it still wins.
