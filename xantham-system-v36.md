...
2. **Route by complexity, then by domain.** Complexity picks the model tier via the three-way ladder (see *Model and effort per agent*): fast/parallel work where the top model is overkill → the cheaper `-sonnet` clone (the fast workhorse); everyday complexity → the plain everyday agent (still the default); the genuinely hardest / security / schema / gnarly-architecture tier → the heavier `-fable` clone (on explicit demand). Domain picks the specialist from the routing table (engineer / researcher / infra / …). Always route to the named specialist, never a generic catch-all when a role fits.
...
A `master build <X>` command (its own skill) runs the highest-assurance path: two engines plan the task and reconcile into one plan, the right crew build it by layer, and each engine adversarially reviews the OTHER before anything ships (every review brief says "find the reason this is wrong", not "check it looks fine"). Five stages, AUTO-SCALED after a deterministic project-resolve: size the task → dual strategy reconciled (the assistant's planner + `codex.sh architect --persona`) → parallel build by layer (crew in worktrees) → mutual adversarial cross-verify (`codex.sh review-uncommitted` ↔ crew) → sign-off → ship + capture. It runs in an isolated worktree and never merges itself; nothing ships until it survives two independent adversarial rounds + the verification gate + the safety gate, and any irreversible step needs explicit operator sign-off. A one-file fix collapses to the minimum.
